...
//! in `colophon`; this crate is argument parsing, session plumbing, and
//! presentation.
//!
...
//!   workspace ([`find_root`]) and drives the library's [`colophon::StdFs`]-backed
//!   engine through the dependency-free [`colophon::block_on`] executor, and the
...
/// ([`colophon::discover`]); this only supplies the real current directory and
/// phrases the two failure modes as CLI diagnostics. Does not warn about config —
/// see [`find_root`].
fn find_root_quiet() -> Result<Ctx, AnyError> {
    let cwd = std::env::current_dir()?;
    match block_on(colophon::discover(&StdFs, &cwd))? {
        colophon::Discovery::Found(d) => Ok(Ctx {
            root_dir: d.root_dir,
            root_doc: d.root_doc,
            registry: d.registry,
            config: d.config,
        }),
        colophon::Discovery::Ambiguous { dir, candidates } => Err(format!(
            "ambiguous workspace root in {}: {} (rename one, or add part_of)",
            dir.display(),
            candidates.join(", ")
        )
        .into()),
        colophon::Discovery::NotFound => Err(
...
fn cmd_rm(path: &str, force: bool, purge: bool) -> CmdResult {
    ...
    let danglers = if ctx.config.recycle_bin && !purge {
        ...
        persist(&ctx, &mut ws)?;
        println!(
            "moved {} to the recycle bin (restore with `colophon restore`)",
            resolved.display()
        );
        danglers
    } else {
